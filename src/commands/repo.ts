...
export class RepoCommand implements Command {
  ...
  private async fetchGeminiResponse(query: string, repoContext: string, options?: CommandOptions): Promise<string> {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${options?.model || this.config.gemini.model}:generateContent?key=${apiKey}`, {
      method: 'POST',
      headers: {
    ...
    try {
      ...
      await pack(process.cwd(), {
        ...
        ignore: {
          ...
          customPatterns: ['**/*.pbxproj', '**/node_modules/**', '**/dist/**', '**/build/**', '**/compile/**', '**/.*/**']
        },
        security: {
